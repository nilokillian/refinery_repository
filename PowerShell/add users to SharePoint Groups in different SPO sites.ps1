This script allows to add several users to different (and existing) SharePoint Groups in different SharePoint Online sites by reading all the information from a CSV file. The script defines a function where the Client Side Object Model (CSOM) is used to add each user read from the CSV file to the specific SharePoint Group in a certain SharePoint Online site. In order to be able to add a user to a group, you have to use the right CSOM objects and follow the right order: first  you have to access the SharePoint groups collection in the site, then you have to get the specific SharePoint group and finally you can add the SharePoint user to that SharePoint group. The CSV file format should be as follows:
